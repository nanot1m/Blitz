...
const codepoints = [...new Set(Object.keys(font.encoding.cmap.glyphIndexMap).map(Number))]
  ...
  .sort((a, b) => a - b);
const capHeight =
  font.tables.os2?.sCapHeight ?? font.charToGlyph("H").getBoundingBox().y2;
const xHeight =
  font.tables.os2?.sxHeight ?? font.charToGlyph("x").getBoundingBox().y2;
...
let atlasSize = 1024;
while (!packGlyphs(atlasSize)) {
  atlasSize *= 2;
  if (atlasSize > 8192) {
    throw new Error("Font atlas overflowed the maximum 8192x8192 size.");
  }
}
...
const metrics = glyphs
  .map(
    (glyph) =>
      `  {${glyph.codepoint}u, ${floatLiteral(glyph.advance)}, ${floatLiteral(glyph.left / fontSize)}, ` +
...
#define BLITZ_FONT_GLYPH_COUNT ${glyphs.length}u
#define BLITZ_FONT_REPLACEMENT_CODEPOINT 65533u
...
#define BLITZ_FONT_CAP_HEIGHT ${floatLiteral(capHeight / font.unitsPerEm)}
#define BLITZ_FONT_X_HEIGHT ${floatLiteral(xHeight / font.unitsPerEm)}
